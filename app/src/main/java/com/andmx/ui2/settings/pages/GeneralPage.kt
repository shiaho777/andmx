...
fun GeneralPage(onBack: () -> Unit) {
    ...
    Scaffold(topBar = { backAppBar("常规", onBack) }) { padding ->
        Column(
            Modifier
                .padding(padding)
            ...
            SettingsGroup("语言与地区") {
                StackedSettingRow(
                    ...
                    SegmentedRow(
                        options = listOf("system" to "系统", "zh" to "中文", "en" to "English"),
                        selected = s.locale,
            ...
            SettingsGroup("交互行为") {
                StackedSettingRow(
                    ...
                    SegmentedRow(
                        options = listOf("queue" to "队列", "guide" to "引导"),
            ...
            SettingsGroup("终端") {
                StackedSettingRow(
                    title = "终端字体",
                    description = "留空时自动继承系统终端字体；填写后作为终端的字体覆盖。"
                ) {
                    OutlinedTextField(
                        value = s.terminalFontFamily,
                        onValueChange = { save(s.copy(terminalFontFamily = it)) },
                        placeholder = { Text("留空自动继承，例如 MesloLGS NF, monospace") },
                        modifier = Modifier.fillMaxWidth(),
                        singleLine = true
                    )
                }
            ...
            SettingsGroup("任务管理") {
                ...
                if (s.taskAutoArchive) {
                    ...
                    StackedSettingRow(
                        ...
                        SegmentedRow(
                            options = listOf(
                                "3" to "3天", "7" to "7天", "14" to "14天", "30" to "30天"
                            ),
                            selected = s.taskAutoArchiveDays.toString(),
                            onSelect = { save(s.copy(taskAutoArchiveDays = it.toInt())) },
                            modifier = Modifier.fillMaxWidth()
                        )
                    }
                }
