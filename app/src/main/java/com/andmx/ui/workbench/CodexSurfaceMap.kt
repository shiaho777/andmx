...
internal fun buildCodexSurfaceMap(
    ...
    val surfaces = listOf(
        CodexSurfaceSpec(
            ...
            command = "/plan",
            manualBasis = "Codex app features: projects, Local/Worktree/Cloud modes, goal progress row and thread automations.",
        ),
        CodexSurfaceSpec(
            title = "会话流",
        ...
        CodexSurfaceSpec(
            ...
            codexPattern = "Codex 通过 Skills、Plugins、MCP、Automations、AGENTS.md、rules 和 hooks 扩展长期工作方式。",
            andmxSurface = "PluginsOverlay / AutomationsOverlay / InstructionStackSummary / McpManager",
            states = listOf("技能可发现", "MCP 待连接", "自动化待配置", "项目指令", "生命周期约束"),
            interactions = listOf("打开工具", "检查 MCP", "查看指令栈", "进入自动化", "把重复反馈固化"),
            acceptance = "可通过 /tools、/instructions、/self-model 看到扩展能力和缺口。",
            command = if (snapshot.mcpServers > 0) "/tools" else "/instructions",
            manualBasis = "Customization docs: AGENTS.md, skills, MCP, automations, plugins, rules and hooks.",
        ),
        CodexSurfaceSpec(
            title = "截图复刻流水线",
